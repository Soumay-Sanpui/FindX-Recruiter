...
const Messages = () => {
        ...
        for (const applicant of applicants) {
          try {
            const conversationResponse =
              await messageAPI.getConversationHistory(
                employer._id,
                applicant.user._id,
            ...
            if (conversationResponse.success) {
              const unreadMessages =
                conversationResponse.messages?.filter(
                  (msg) => !msg.isRead && msg.fromModel === "User"
                ) || [];
              unreadCounts[applicant.user._id] = unreadMessages.length;
            }
          } catch (error) {
            console.error(
              `Error loading unread count for applicant ${applicant.user._id}:`,
              error
            );
          }
        }
  ...
  // Refresh unread counts after a delay to allow backend to mark messages as read
  const refreshUnreadCounts = useCallback(async () => {
    if (!employer?._id || applicants.length === 0) return;

    // Wait a bit for the backend to process the automatic marking as read
    setTimeout(async () => {
      try {
        const response = await messageAPI.getUnreadMessageCount(employer._id);
        if (response.success) {
          ...
          // Refresh individual unread counts for each applicant
          const unreadCounts = {};
          for (const applicant of applicants) {
            try {
              const conversationResponse =
              ...
              console.error(
                `Error refreshing unread count for applicant ${applicant.user._id}:`,
                error
              );
            }
          }
  ...
  useEffect(() => {
    if (applicants.length > 0) {
      loadUnreadCounts();
      // Also refresh unread counts after a delay
      setTimeout(() => refreshUnreadCounts(), 1000);
    }
  }, [applicants, loadUnreadCounts, refreshUnreadCounts]);
  ...
  const loadConversationHistory = useCallback(
    async (applicant, showLoading = true) => {
      if (!applicant || !employer) return;

      const cacheKey = `${employer._id}-${applicant.user._id}-${
        applicant.jobId || jobId
      }`;
      const now = Date.now();
      ...
      try {
        ...
        const conversationResponse = await api.get(
          `/messages/employer/conversation/${employer._id}/${applicant.user._id}/${jobIdToUse}`
        );
        ...
        if (conversationResponse.data?.success) {
          ...
          newMessages.forEach((msg) => {
            if (!msg.isRead && msg.fromModel === "User") {
          ...
          // Refresh unread counts after loading conversation
          setTimeout(() => refreshUnreadCounts(), 1000);
  ...
  useEffect(() => {
    ...
    if (!employer?.messagesAllowed) {
      setError(
        "Messaging is not enabled for your account. Please enable messaging in your settings to use this feature."
      );
      setLoading(false);
      return;
    }
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        // Update cache
        const cacheKey = `${employer._id}-${selectedApplicant.user._id}-${
  ...
  const handleMarkAllAsRead = useCallback(async () => {
    ...
    try {
      console.log("Marking all messages as read for employer:", employer._id);
      const result = await messageAPI.markAllMessagesAsRead(employer._id);
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="bg-white rounded-lg shadow-md p-8 text-center">
            ...
            <p className="text-gray-600 mb-4">
              {error || jobError?.message || "Something went wrong"}
            </p>
            <button
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-blue-600 text-sm">
                  Select an applicant to view their messages or click "Mark All
                  Read" to clear all notifications.
                </p>
              </div>
            </div>
          </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <div>
                        <h3 className="font-semibold text-gray-900 flex items-center">
                          ...
                          {applicantUnreadCounts[selectedApplicant.user._id] >
                            0 && (
                            <span className="ml-2 bg-red-500 text-white text-xs px-2 py-1 rounded-full">
                              {
                                applicantUnreadCounts[
                                  selectedApplicant.user._id
                                ]
                              }{" "}
                              unread
                        ...
                        >
                          <div className="max-w-xs lg:max-w-md">
                            ...
                            >
                              {message.fromModel === "Employer"
                                ? "You"
                                : selectedApplicant.user?.name || "User"}
