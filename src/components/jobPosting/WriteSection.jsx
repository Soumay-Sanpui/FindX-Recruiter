...
const WriteSection = ({formData, handleChange, handleStageChange}) => {
    ...
    const toggleSectionDropdown = (sectionKey) => {
        setShowSectionDropdowns(prev => ({...prev, [sectionKey]: !prev[sectionKey]}));
    };

    const toggleAdvancedSectionDropdown = (sectionKey) => {
        setShowAdvancedSectionDropdowns(prev => ({...prev, [sectionKey]: !prev[sectionKey]}));
    };
   ...
   const handleQuestionSelect = (e, question) => {
        ...
        if (isChecked) {
            ...
            handleChange({target: {name: 'mandatoryQuestions', value: updatedMandatory}});
        }
    ...
    const handleOptionSelect = (question, option, isSelected) => {
        ...
        if (isSelected) {
            ...
            if (updatedOptions[question].length === 0) delete updatedOptions[question];
        }

        setSelectedOptions(updatedOptions);
        handleChange({target: {name: 'selectedOptions', value: updatedOptions}});
    };
    ...
    const handleUseCompanyLogo = () => {
        if (employer && employer.companyLogo) {
            setLogoFile(null);
            handleChange({target: {name: 'companyLogo', value: employer.companyLogo}});
        } else {
        ...
        <div className="flex flex-col lg:flex-row bg-white shadow-lg border-2 border-blue-800">
            ...
            <div className="lg:w-3/4 p-8">
                <div className="mb-8">
                    ...
                    <p className="text-gray-600">Get started with a draft job description and summary, based on your
                        role information.</p>
                </div>
                            ...
                            onClick={() => {
                                if (shortDescriptionInput.trim()) {
                                    if (shortDescriptionInput.includes(',')) {
                                        const newItems = shortDescriptionInput.split(',').map(item => item.trim()).filter(item => item !== '');
                                        const updatedItems = [...(formData.shortDescription || []), ...newItems];
                                        handleChange({target: {name: 'shortDescription', value: updatedItems}});
                                    } else {
                ...
                {formData.showShortDescription && formData.shortDescription && (
                    <div className="mb-6">
                        <label htmlFor="shortDescriptionPreview" className="block text-gray-700 font-medium mb-2">Job
                ...
                <div className="mb-6">
                    <label htmlFor="sellingPoints" className="block text-gray-700 font-medium mb-2">Key selling
                        points <span className="text-gray-500">(optional)</span></label>
                    <p className="text-gray-500 text-sm mb-2">Enter 3 key selling points to attract candidates to view
                        your role.</p>
                        ...
                        {[0, 1, 2].map((index) => (
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    handleChange({target: {name: 'sellingPoints', value: updatedPoints}});
                                }}
                            />
                        ))}
                    </div>
                ...
                <div className="mb-6">
                    <label className="block text-gray-700 font-medium mb-2">Job keywords <span
                        className="text-gray-500">(optional)</span></label>
                    <p className="text-gray-500 text-sm mb-2">Add relevant keywords to help candidates find your job.
                        You can add multiple keywords separated by commas.</p>
                    <div className="flex gap-2 mb-2">
                        <input
                            type="text"
                                ...
                                if (e.key === 'Enter') {
                                    ...
                                    if (searchQuery.trim()) {
                                        if (searchQuery.includes(',')) {
                                            const newKeywords = searchQuery.split(',').map(keyword => keyword.trim()).filter(keyword => keyword !== '');
                                            const updatedKeywords = [...(formData.jobKeywords || []), ...newKeywords];
                                            handleChange({target: {name: 'jobKeywords', value: updatedKeywords}});
                                        } else {
                        ...
                        <button
                            ...
                            onClick={() => {
                                if (formData.category && CONFIG.categoryKeywords[formData.category]) {
                                    ...
                                    const newKeywords = suggestedKeywords.filter(keyword => !formData.jobKeywords?.includes(keyword));
                ...
                {/* Job Skills Section */}
                <div className="mb-6">
                    <label className="block text-gray-700 font-medium mb-2">Job Skills <span
                        className="text-gray-500">(optional)</span></label>
                    <p className="text-gray-500 text-sm mb-2">Enter the key skills required for this position. Separate
                        multiple skills with commas.</p>
                    <div className="flex gap-2 mb-2">
                        <input
                            type="text"
                            className="w-2/3 border border-gray-300 p-3 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                ...
                                if (e.key === 'Enter') {
                                    ...
                                    if (jobSkillsInput.trim()) {
                                        if (jobSkillsInput.includes(',')) {
                                            const newSkills = jobSkillsInput.split(',').map(skill => skill.trim()).filter(skill => skill !== '');
                                            const updatedSkills = [...(formData.jobSkills || []), ...newSkills];
                                            handleChange({target: {name: 'jobSkills', value: updatedSkills}});
                                        } else {
                ...
                <div className="mb-6">
                    <label className="block text-gray-700 font-medium mb-2">Company Logo <span
                        className="text-gray-500">(optional)</span></label>
                    ...
                    {formData.companyLogo && (
                        <div
                            className="border border-gray-300 p-4 flex items-center justify-center h-32 bg-gray-50 mb-4">
                            <img
                ...
                {/* Job Banner Section */}
                <div className="mb-6">
                    <label className="block text-gray-700 font-medium mb-2">Job banner <span
                        className="text-gray-500">(optional)</span></label>
                    <p className="text-gray-500 text-sm mb-2">Add a banner image URL to make your job post stand
                        out.</p>
                    <input
                        type="text"
                        className="w-full border border-gray-300 p-3 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                {/* Video Section */}
                <div className="mb-8">
                    <label className="block text-gray-700 font-medium mb-2">Video <span
                        className="text-gray-500">(optional)</span></label>
                    <p className="text-gray-500 text-sm mb-2">Enter a link to your ad with a YouTube link. The video
                        will appear at the bottom of your ad.</p>
                    <input
                        type="text"
                        className="w-full border border-gray-300 p-3 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                        ...
                                        <div key={sectionKey}
                                            ...
                                            >
                                                <div className="flex items-center">
                                                    <div
                                                        className="flex-shrink-0 w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center mr-3">
                                                                ...
                                                                {questionObj.options && questionObj.options.length > 0 && showQuestionOptions[`${sectionKey}-${questionIndex}`] && (
                                                                    <div className="mt-2">
                                                                        <div
                                                                            ...
                                                                            <p className="text-xs font-medium text-gray-700 mb-2">Select
                                                                                answer options to show to
                                                                                candidates:</p>
                                                                            <div className="space-y-2">
                            ...
                            <div className="mt-4 bg-purple-50 p-4 rounded-md border border-purple-100">
                                {Object.keys(filteredAdvancedQuestions).length === 0 ? (
                                    <div className="text-center py-8">
                                        ...
                                        <h3 className="text-lg font-medium text-gray-800 mb-1">No Advanced Questions
                                            Available</h3>
                                    ...
                                    <>
                                        {formData.category && (
                                            <div className="mb-4 p-3 bg-purple-100 border border-purple-200 rounded-md">
                                                <div className="flex items-center">
                                                    <svg className="w-5 h-5 text-purple-600 mr-2" fill="none"
                                                         stroke="currentColor" viewBox="0 0 24 24">
                                                        <path strokeLinecap="round" strokeLinejoin="round"
                                                              strokeWidth="2"
                                            ...
                                            {Object.entries(filteredAdvancedQuestions).map(([sectionKey, section]) => (
                                                <div key={sectionKey}
                                                    ...
                                                    >
                                                        <div className="flex items-center">
                                                            <div
                                                                className="flex-shrink-0 w-8 h-8 bg-purple-600 rounded-full flex items-center justify-center mr-3">
                                                                        ...
                                                                        <div
                                                                            ...
                                                                            {formData.jobQuestions && formData.jobQuestions.includes(questionObj.question) && (
                                                                                <div
                                                                                    className="flex items-center space-x-2">
                                                                                    <span
                                                                                        className="text-xs text-gray-600">Make Mandatory:</span>
                                                                                    <label
                                                                                        className="relative inline-flex items-center cursor-pointer">
                                                                                        <input
                                                                                            type="checkbox"
                                                                                        ...
                                                                                        />
                                                                                        <div
                                                                                            className="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-purple-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-purple-600"></div>
                                                                                    </label>
                                                                                </div>
                                                                            )}
                                                                        </div>
                    ...
                    <div className="mb-4 bg-blue-50 p-4 rounded-md border border-blue-100">
                        <div className="flex items-start">
                            ...
                            <p className="ml-2 text-sm text-blue-700">
                                Your questions and answers can't be changed after you post your job ad. This is to
                                ensure
                    ...
                    {premiumSelected ? (
                        ...
                        <>
                            ...
                            <div className="text-sm space-y-1 mb-4">
                                ...
                                <p className="mt-2 text-gray-600">Add your logo, cover photo, embedded video to stand
                                    out</p>
                                <p className="text-gray-600">LinkedIn, Career profile access (if provided by
                                    candidate)</p>
                            </div>
                        </>
                    )}
