...
const WriteSection = ({ formData, handleChange, handleStageChange }) => {
  ...
  const { employer } = useEmployerStore();
    ...
    // pricing elements - 
  const [premiumSelected, _setPremiumSelected] = useState(
    formData?.premiumListing ?? false
  );
  const [immediateStartSelected, _setImmediateStartSelected] = useState(
    formData?.immediateStart ?? false
  );
  const [notificationOption, _setNotificationOption] = useState(
  ...
  const [showAdvancedSectionDropdowns, setShowAdvancedSectionDropdowns] =
    useState({
      business: false,
      technology: false,
      professional: false,
  ...
  // Dynamic notification pricing based on count (same as AdTypesSection)
  ...
  const basicQuestionsWithOptions = [
    ...
    {
      question:
        "Are you legally allowed to work with children or vulnerable people?",
      options: [
    ...
    {
      question:
        "Do you have experience working in remote or hybrid environments?",
      options: [
        "Yes - fully remote",
    ...
    {
      question:
        "Are you open to completing additional training if the role requires it?",
      options: [
    ...
    {
      question:
        "Have you completed any safety training or certifications (e.g., First Aid, White Card)?",
      options: [
        "Yes - First Aid",
        "Yes - White Card",
    ...
    {
      question:
        "Are you comfortable using workplace technology/tools (e.g., Zoom, Slack, CRM)?",
      options: [
    ...
    {
      question:
        "Do you have experience handling cash or point-of-sale (POS) systems?",
      options: [
    ...
    {
      question:
        "Do you have previous team management or leadership experience?",
      options: [
    ...
    {
      question:
        "Do you have experience working in multicultural teams or diverse environments?",
      options: [
        "Yes – regularly",
        "Yes – occasionally",
    ...
    {
      question:
        "Are you physically fit for roles that require manual labour or standing for long periods?",
      options: [
    ...
    {
      question:
        "Are you comfortable using or wearing personal protective equipment (PPE)?",
      options: [
    ...
    {
      question:
        "Are you comfortable working in environments that require confidentiality (e.g., healthcare, legal)?",
      options: [
    ...
    {
      question:
        "Have you worked in compliance-heavy industries (e.g., finance, insurance, healthcare)?",
      options: [
...

    {
      ...
      options: [], // This question is handled via UI dropdown/input
    },
  ...
  const toggleQuestionOptions = (sectionKey, questionIndex) => {
    ...
    setShowQuestionOptions((prev) => ({ ...prev, [key]: !prev[key] }));
  };
  ...
  const handleQuestionSelect = (e, question) => {
    ...
    if (isChecked) {
      ...
      handleChange({
        target: { name: "mandatoryQuestions", value: updatedMandatory },
      });
    }

    handleChange({ target: { name: "jobQuestions", value: updatedQuestions } });
  };
  ...
  const handleOptionSelect = (question, option, isSelected) => {
    ...
    if (isSelected) {
      ...
      updatedOptions[question] = updatedOptions[question].filter(
        (opt) => opt !== option
      );
    ...
    handleChange({
      target: { name: "selectedOptions", value: updatedOptions },
    });
  };
  ...
  const validateQuestionsWithOptions = () => {
    ...
    formData.jobQuestions.forEach((question) => {
      ...
      const allBasicQuestions = Object.values(questionSections)
        .flat()
      ...
      if (!questionObj) {
        const allAdvancedQuestions = Object.values(filteredAdvancedQuestions)
          .map((section) => section.questions)
          .flat();
      ...
      if (
        ...
        if (
          ...
          selectedOptions[question].length === 0
        ) {
    ...
    if (questionsWithoutOptions.length > 0) {
      const questionList = questionsWithoutOptions
        .map((q) => `• ${q}`)
        .join("\n");
      alert(
        `Please select answer options for the following questions before continuing:\n\n${questionList}`
      );
      return false;
    }
  ...
  const handleUseCompanyLogo = () => {
    if (employer && employer.companyLogo) {
      setLogoFile(null);
      handleChange({
        target: { name: "companyLogo", value: employer.companyLogo },
      });
    } else {
      alert(
        "Unable to fetch company logo! Please make sure you have set a company logo in your profile settings."
      );
    }
  };
      ...
      <div className="lg:w-3/4 p-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Get started with a draft job description and summary, based on your
            role information.
          </p>
        </div>
        ...
        <div className="mb-6">
          ...
          <textarea
            className={`w-full border p-3 h-32 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-sm mb-2">
            Write a compelling statement about your role in white to more
            candidates.
          </p>
          <textarea
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-500 text-sm mb-2">
                Write a short comma separated (,) list of key points about your
                role.
              </p>
            </div>
            <button
              ...
              onClick={() => {
                if (shortDescriptionInput.trim()) {
                  if (shortDescriptionInput.includes(",")) {
                    ...
                    handleChange({
                      target: { name: "shortDescription", value: updatedItems },
                    });
                  } else {
                    const updatedItems = [
                      ...(formData.shortDescription || []),
                ...
                {formData.shortDescription.map((item, index) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() => {
                        const updatedItems = formData.shortDescription.filter(
                          (_, i) => i !== index
                        );
                        handleChange({
                          target: {
        ...
        {formData.showShortDescription && formData.shortDescription && (
          <div className="mb-6">
            <label
              htmlFor="shortDescriptionPreview"
              className="block text-gray-700 font-medium mb-2"
            >
              Job short description preview
            </label>
        ...
        <div className="mb-6">
          <label
            ...
            className="block text-gray-700 font-medium mb-2"
          >
            Key selling points <span className="text-gray-500">(optional)</span>
          </label>
          <p className="text-gray-500 text-sm mb-2">
            Enter 3 key selling points to attract candidates to view your role.
          </p>
          <div className="space-y-2">
            {[0, 1, 2].map((index) => (
              <input
                ...
                value={
                  formData.sellingPoints && formData.sellingPoints[index]
                    ? formData.sellingPoints[index]
                    : ""
                }
                onChange={(e) => {
                  ...
                  handleChange({
                    target: { name: "sellingPoints", value: updatedPoints },
                  });
                }}
              />
            ))}
        ...
        <div className="mb-6">
          <label className="block text-gray-700 font-medium mb-2">
            Job keywords <span className="text-gray-500">(optional)</span>
          </label>
          <p className="text-gray-500 text-sm mb-2">
            Add relevant keywords to help candidates find your job. You can add
          ...
          <div className="flex gap-2 mb-2">
                ...
                if (e.key === "Enter") {
                  ...
                  if (searchQuery.trim()) {
                    if (searchQuery.includes(",")) {
                      ...
                      handleChange({
                        target: { name: "jobKeywords", value: updatedKeywords },
                      });
                    } else {
                      const updatedKeywords = [
                        ...(formData.jobKeywords || []),
            ...
            <button
              ...
              onClick={() => {
                if (
                  formData.category &&
                  CONFIG.categoryKeywords[formData.category]
                ) {
              ...
              title={
                !formData.category
                  ...
                  : !CONFIG.categoryKeywords[formData.category]
                  ? "No suggested keywords for this category"
                  : "Generate keywords based on category from config file"
              }
            >
              <Sparkles />
              Auto Generate
          ...
          {formData.jobKeywords &&
            Array.isArray(formData.jobKeywords) &&
            formData.jobKeywords.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {formData.jobKeywords.map((keyword, index) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="ml-2 text-blue-600 hover:text-blue-800"
                      onClick={() => {
                        const updatedKeywords = formData.jobKeywords.filter(
                          (_, i) => i !== index
                        );
                        handleChange({
                          target: {
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-sm mb-2">
            Enter the key skills required for this position. Separate multiple
            skills with commas.
          </p>
          <div className="flex gap-2 mb-2">
            <input
                ...
                if (e.key === "Enter") {
                  ...
                  if (jobSkillsInput.trim()) {
                    if (jobSkillsInput.includes(",")) {
                      ...
                      handleChange({
                        target: { name: "jobSkills", value: updatedSkills },
                      });
                    } else {
                      const updatedSkills = [
                        ...(formData.jobSkills || []),
          ...
          {Array.isArray(formData.jobSkills) &&
                ...
                {formData.jobSkills.map((skill, index) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="ml-2 text-blue-600 hover:text-blue-800"
                      onClick={() => {
                        const updatedSkills = formData.jobSkills.filter(
                          (_, i) => i !== index
                        );
                        handleChange({
                          target: {
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-sm mb-2">
            Enter your logo URL or use your company logo.
          </p>
        ...
        <div className="mb-6">
          <label className="block text-gray-700 font-medium mb-2">
            Job banner <span className="text-gray-500">(optional)</span>
          </label>
          <p className="text-gray-500 text-sm mb-2">
            Add a banner image URL to make your job post stand out.
          </p>
          <input
            type="text"
            className="w-full border border-gray-300 p-3 focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 text-sm mb-2">
            Enter a link to your ad with a YouTube link. The video will appear
            ...
            >
              <div>
                ...
                <p className="text-gray-500 text-sm">
                  Essential questions that you can choose to include in your
                  application form
                </p>
              </div>
              <div className="flex-shrink-0 ml-4">
                <svg
                  className={`w-6 h-6 text-gray-600 font-bold transition-transform duration-200 ${
                    showBasicQuestions ? "rotate-180" : ""
                  }`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                ...
                <div className="space-y-4">
                  {Object.entries(questionSections).map(
                    ([sectionKey, section]) => (
                      <div
                        key={sectionKey}
                        className="bg-white rounded-md border border-blue-200 overflow-hidden"
                      >
                        {/* Section Header */}
                        <div
                                ...
                                >
                                  <input
                                    ...
                                    checked={
                                      ...
                                      formData.jobQuestions.includes(
                                  ...
                                  <div className="flex-1">
                                    <label
                                      htmlFor={`basic-question-${sectionKey}-${questionIndex}`}
                                          ...
                                          <div className="flex items-center space-x-2">
                                            ...
                                            <label className="relative inline-flex items-center cursor-pointer">
                                              <input
                                                ...
                                                checked={mandatoryQuestions.includes(
                                                  questionObj.question
                                                )}
                                                onChange={(e) =>
                                        ...
                                        <div className="mt-2">
                                          <div className="bg-white rounded-md p-3 border border-gray-300">
                                            <p className="text-xs font-medium text-gray-700 mb-2">
                                              Select answer options to show to
                                              candidates:
                                            </p>
                                            <div className="space-y-2">
                                                ...
                                                (option, optionIndex) => (
                                                  ...
                                                  >
                                                    <input
                                                      ...
                                                      key={`basic-option-${sectionKey}-${questionIndex}-${optionIndex}-${
                                                        selectedOptions[
                                                          questionObj.question
                                                        ]
                                                          ? selectedOptions[
                ...
                {Object.keys(filteredAdvancedQuestions).length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-lg font-medium text-gray-800 mb-1">
                      No Advanced Questions Available
                    </h3>
                  ...
                  <>
                      ...
                      <div className="mb-4 p-3 bg-purple-100 border border-purple-200 rounded-md">
                        <div className="flex items-center">
                          ...
                          <span className="text-sm text-purple-700 font-medium">
                            Showing questions relevant to "{formData.category}"
                            category
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="space-y-4">
                      {Object.entries(filteredAdvancedQuestions).map(
                        ([sectionKey, section]) => (
                          <div
                            key={sectionKey}
                            className="bg-white rounded-md border border-purple-200 overflow-hidden"
                          >
                            {/* Section Header */}
                            <div
                              className="flex items-center justify-between cursor-pointer p-4 bg-gray-50 hover:bg-gray-100 transition-colors"
                              onClick={() =>
                                toggleAdvancedSectionDropdown(sectionKey)
                              }
                            >
                              <div className="flex items-center">
                                ...
                                {section.questions.map(
                                  (questionObj, questionIndex) => (
                                    ...
                                    >
                                      ...
                                      <div className="flex-1">
                                        <label
                                          htmlFor={`advanced-question-${sectionKey}-${questionIndex}`}
                                          className="text-gray-700 cursor-pointer text-sm font-medium"
                                        >
                                          {questionObj.question}
                                        </label>
                                                    ...
                                                    (option, optionIndex) => (
                                                      ...
                                                      >
                                                        <input
                                                          ...
                                                          key={`advanced-option-${sectionKey}-${questionIndex}-${optionIndex}-${
                                                            selectedOptions[
                                                              questionObj
                                                                .question
                                                            ]
          ...
          <div className="mb-4 bg-blue-50 p-4 rounded-md border border-blue-100">
            <div className="flex items-start">
              ...
              <p className="ml-2 text-sm text-blue-700">
                Your questions and answers can't be changed after you post your
                job ad. This is to ensure a fair application process for all
                candidates.
              </p>
            </div>
          </div>
      ...
      <div className="block lg:hidden mt-10 border border-gray-300 shadow-md rounded-md bg-white">
        ...
        <div className="p-4">
          ...
          {notificationOption !== "none" && (
            <>
                                          <div className="flex justify-between mt-4 mb-2">
                                <span>Notification Package ({formData.notificationCount || 100} candidates):</span>
